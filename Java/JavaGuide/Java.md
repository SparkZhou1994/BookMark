...
- 系统信息
###### 内存操作
需要注意，通过这种方式分配的内存属于 堆外内存 ，是无法进行垃圾回收的，需要我们把这些内存当做一种资源去手动调用freeMemory方法进行释放，否则会产生内存泄漏。通用的操作内存方式是在try中执行对内存的操作，最终在finally块中进行内存的释放。可以通过JVM参数配置堆外内存大小，-XX:MaxDirectMemorySize
####### 为什么要使用堆外内存？
...
所谓扰动函数指的就是 HashMap 的 hash 方法。使用 hash 方法也就是扰动函数是为了防止一些实现比较差的 hashCode() 方法 换句话说使用扰动函数之后可以减少碰撞。有默认大小16。
###### JDK 1.8 之后
JDK1.8 之后在解决哈希冲突时有了较大的变化，当链表长度大于阈值（默认为 8）（将链表转换成红黑树前会判断，如果当前数组的长度小于 64，那么会选择先进行数组扩容，而不是转换为红黑树）时，将链表转化为红黑树，以减少搜索时间。没有默认大小，在put()后进行判断。
...
为了更灵活管理内存，讲内存继续多重映射，把一块物理内存映射成3个虚拟内存。
添加染色指针，将GC信息添加的指针上，相比之前放在对象头的Mark Word中，更丰富。
内存分布，采用region，分为大小不同的三种，2MB，32MB，N*2MB
在对象标记和转移时用读屏障，提高GC并发能力。
### 类文件结构详解（重点）
